$(document).ready(function(){
    ...
    if (amigoEstado == "rechazado") {
		$('#cuerpoPublicaciones').append("<div class='w3-container w3-center w3-card w3-white w3-round w3-margin'><br><p>Esta persona a rechazado su solicitud. Ya no podra ver sus publicaciones</p></div>");
	}else{
		$.ajax({
			url: baseurl+'Publicacion/obtenerPuclicacionesUnico',
			type: 'POST',
			data: {id_cuenta:id_cuenta,estado:amigoEstado},
	...
	$(window).on("scroll", function() {
		if (!banderaScroll || amigoEstado == "pendiente" || (banderaPerfil != 'NO' && amigoEstado == '') || amigoEstado == "rechazado") {
			return
		}
		console.log(limite);
		if (bandera) {
		    if($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
		        ...
		        $.ajax({
					url: baseurl+'Publicacion/obtenerPuclicacionesUnico',
					type: 'POST',
					data: {limite:limite,id_cuenta:id_cuenta,estado:amigoEstado},
					success: function(resultado){
						resultado = JSON.parse(resultado);
						if (resultado.estado == 'vacio') {
							$('#cuerpoPublicaciones').append(resultado.publicacion);
